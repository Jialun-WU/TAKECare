...
class My_CLUBSample(nn.Module):  # Sampled version of the CLUB estimator
    def __init__(self, x_dim, y_dim, hidden_size):
        super(My_CLUBSample, self).__init__()
        self.p_mu = nn.Sequential(nn.Linear(x_dim, hidden_size//2),
                                       nn.ReLU(),
                                       nn.Linear(hidden_size//2, y_dim))
    ...
    def forward(self, x_samples, y_samples):
        ...
        positive = - (mu - y_samples)**2 / 2. / logvar.exp()
        negative = - (mu - y_samples[random_index])**2 / logvar.exp()
        upper_bound = self.relu(positive.sum(dim = -1) - negative.sum(dim = -1))
        upper_bound = (upper_bound).mean()
        return upper_bound/2.
        # return torch.log(upper_bound/2.)
        # return nn.ReLU(upper_bound/2.)

    def learning_loss(self, x_samples, y_samples):
        return - self.loglikeli(x_samples, y_samples)

class MyCLUB(nn.Module):  # CLUB: Mutual Information Contrastive Learning Upper Bound
    '''
        This class provides the CLUB estimation to I(X,Y)
        Method:
            forward() :      provides the estimation with input samples  
